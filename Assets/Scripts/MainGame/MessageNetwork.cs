...
public class MessageNetwork : MonoBehaviour {
    ...
    void UpdateRooms()
    {
        ...
        /*//int maxRoom = PhotonNetwork.GetRoomList().;
        //List<RoomInfo> roomList = PhotonNetwork.CurrentRoom.GetRoomList();
        TypedLobby typedLobby = new TypedLobby("lobbyName", LobbyType.Default);
        string roomName = "myRoomName";
        RoomInfo[] roomList = new RoomInfo[0];
        roomList.adds(PhotonNetwork.GetCustomRoomList(typedLobby, roomName));
        
        int maxRoom = roomList.Length;
        ...
        for (int i = 0; i < maxRoom; i++)
        {
            ...
            textMember.text = roomList[i].PlayerCount + "/" + roomList[i].MaxPlayers;
            textName.text = roomList[i].Name;

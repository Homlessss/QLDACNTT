...
public class NetworkMagnager : MonoBehaviour {
    ...
    {
        transform.GetComponent<PhotonView>().RPC("SendKickPlayer", RpcTarget.Others, Modules.listNamePlayer[1]);
    }
    ...
    public void StartGame()
    {
        ...
        transform.GetComponent<PhotonView>().RPC("SendSyncServer", RpcTarget.Others);
    }

    public void SyncAvatar()
    {
        if (Modules.fbMyAvatar != null)
            transform.GetComponent<PhotonView>().RPC("SendSyncAvatar", RpcTarget.Others, Modules.fbMyAvatar.texture.EncodeToJPG());
        else transform.GetComponent<PhotonView>().RPC("SendNullAvatar", RpcTarget.Others);
    }
    ...
    {
        if (PhotonNetwork.IsMasterClient) return;
        transform.GetComponent<PhotonView>().RPC("AskJoinRoom", RpcTarget.MasterClient, Modules.namePlayOnline);
        print("join room");
    }
    ...
    void OnPlayerLeftRoom(Photon.Realtime.Player otherPlayer)//xu ly ca 2 phia server va client
    {
    ...
    void AskJoinRoom(string namePlayer)//xu ly khi client gui du lieu dang ky join len server
    {
        ...
        transform.GetComponent<PhotonView>().RPC("SendJoinRoom", RpcTarget.Others, ImportDataArray(Modules.listNamePlayer, ";"));
    }
    ...
    void SendSyncServer()//gan bien vao doi tuong view control o client
    {
        ...
        syncWrite.GetComponent<SendViewEnemyNew>().isWriting = true;
        syncWrite.GetComponent<SendViewEnemyNew>().CallStart();
        transform.GetComponent<PhotonView>().RPC("SendSyncClient", RpcTarget.MasterClient);
    }
    ...
    void SendSyncClient()//gan bien vao doi tuong view control o server
    {
        ...
        transform.GetComponent<PhotonView>().RPC("SendStartGame", RpcTarget.All);
    }
